...
public class DriveDistanceCommand2 extends CommandBase {
   ...
   * @param timeThreshold     the time threshold (in ms) that specifies how long the robot
   *                          needs to be close to the target for the comletion of
   *                          the {@code DriveDistanceCommand2} (e.g., 100)
   * @param maxStride         the maximum distance (e.g., 0.08) that the robot can travel per
   *                          tick (20 ms)
   * @param convergenceRatio  the maximum ratio (e.g., 0.6) that the robot can travel per tick
   *                          (20ms) relative
   *                          to the distance between the robot and the target
   * @param maxSpeed          the maximum speed (e.g., 0.3) at which the robot is allowed to
  ...
  /**
   * Is called every time the scheduler runs while this {@code DriveDistanceCommand2} is scheduled.
   */
  @Override
  public void execute() {
    double leftSpeed = m_convergenceRatio * (m_distance - DriveSubsystem.get().getLeftEncoderPosition()) / m_maxStride;
    double rightSpeed = m_convergenceRatio * (m_distance - DriveSubsystem.get().getRightEncoderPosition()) / m_maxStride;
    ...
    System.out.println(AprilTagSubsystem.get().m_z + ", " + DriveSubsystem.get().getLeftEncoderPosition() + ", "
        + DriveSubsystem.get().getRightEncoderPosition() + ", " + leftSpeed + ", " + rightSpeed);
  }
   ...
   * @return {@code true} if this {@code DriveDistanceCommand2} is completed; {@code false} otherwise
   */
  @Override
