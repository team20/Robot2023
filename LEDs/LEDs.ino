  ...
  uint32_t LowerRainbowPartyFunTime(int c, int i) {  //Rainbow but blinking
    if (c % 2 == 0) return (RainbowColor(c / 2, i));
  ...
  void receiveEvent(int howMany) {
    ...
    if (pattern == 8 && Wire.available()) {  //for code 90 reset, check if new alliance color being set
      ...
      if (x == 11) {                          //red alliance
        teamColor = upperStrip.Color(255, 0, 0);
        teamColor = lowerStrip.Color(255, 0, 0);
      } else if (x == 12) {  //blue alliance
        teamColor = upperStrip.Color(0, 255, 0);
        teamColor = lowerStrip.Color(0, 255, 0);
      }
    }
  }